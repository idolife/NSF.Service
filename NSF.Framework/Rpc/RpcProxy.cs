...
namespace NSF.Framework.Rpc
{
    /// <summary>
    /// RPC服务客户端代理。
    /// </summary>
    public class RpcProxy : MultiTask
    {
        TcpClient _Peer;
        IPEndPoint _Remote;
        DataBlock _PullCache;
        List<RpcCallStub> _Waiting;

        public RpcProxy(IPEndPoint remote)
        {
            _Remote = remote;
            _PullCache = new DataBlock(1024 * 8);
        ...
        {
            /// 创建一个调用任务进入队列
            RpcCallJob<R> job = RpcUtil.BuildRpcCallJob<R>(req);
            /// 创建一个关联任务的超时任务
            _Waiting.Add(job);
            Put(Task.Delay(5000), job, OnTimeout);

            /// 外发远程调用请求
            if (_Peer != null && _Peer.Connected)
            {
                DataBlock push = new DataBlock(1024);
                /// 创建一个关联任务的发送任务
                RpcUtil.EncodeRpcCallInfo(job.Info, push);
        ...
        {
            /// 创建一个调用任务进入队列
            RpcCallStub job = RpcUtil.BuildRpcCallStub(req);
        ...
        protected void ProcessRecv()
        {
            ...
            {
                /// 解析应答消息
                RpcRespInfo resp = RpcUtil.DecodeRpcRespInfo(_PullCache);
                /// 解析完毕或者当前不足一个完整消息
                if (resp == null)
                    break;
